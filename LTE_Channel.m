...
%     numFramesPerDopplerType = 100;
    ...
    %     numberOfBits = K*L*P*2; 
      ...
      for Doppler_idx = 1:numDopplerTypes
        ...
        for Delay_idx = 1:numDelay
          cfg.DelayProfile = Delays(Delay_idx);      % EVA delay spread

          for fn = 1:numFramesPerDelayType/10
            txGrid = [];
    ...
    %             % demodulation
        ...
        %                 frame(:,1) = rxWaveform;
                        add(frameStore, subframe_f, DelayTypes(Delay_idx));
                    elseif domain == 1 % time-domain
                        subframe_t = rxWaveform(1 + spf * (s - 1) : spf * s, :);
                        add(frameStore, subframe_t, DelayTypes(Delay_idx));
                    else % freq+time
            ...
            %             add(frameStore, frame, DopplerTypes(Doppler));
                        add(frameStore, frame, DelayTypes(Delay_idx));
                    end
                end
    %         end
          end
    ...
    save('LTE' + string(domain) + '_Delay_500.mat', 'rxTraining', 'rxTrainingLabel', 'rxValidation', 'rxValidationLabel', 'rxTest', 'rxTestLabel', '-v7.3');
elseif mode < 100 % pred if 5,10 or 15;  >100 compute pred statistics only
    disp("predicting");
    load('LTE' + string(domain) + '_Delay_500.mat', 'rxTraining', 'rxTrainingLabel', 'rxValidation', 'rxValidationLabel', 'rxTest', 'rxTestLabel');
...
%                     disp("trial: " + string(trial));
            ...
            if scheme == 1  % binary prediction   
                ...
                disp("Window: " + WINDOWS(window) + " Delay: "+boundary(kk)+" AccuracyI: " + testAccuracyI);                
            else
                ...
                for i = 1:numDelay
                    ...
                    disp("Window: " + WINDOWS(window) + " Delay: "+Dopplers(i)+" AccuracyS: "+testAccuracyS);
                end            
            end
        end
 end    
%     save('../../../work/bal718/LTE_Doppler_predAll_' + string(mode) + '_' + string(type) + '_' + string(domain) + '_' + string(scheme) + '.mat',  'rxTestPredAll', 'rxTestLabel', '-v7.3');
   save('LTE_Delay_predAll_' + string(mode) + '_' + string(type) + '_' + string(domain) + string(scheme) + '.mat',  'rxTestPredAll', 'rxTestLabel', '-v7.3');
end
...
%         load('LTE_Delay_predAll_' + string(mode) + '_' + string(type) + '_' + string(domain) + '_' + string(scheme) + '.mat', 'rxTestPredAll', 'rxTestLabel');
%     end
